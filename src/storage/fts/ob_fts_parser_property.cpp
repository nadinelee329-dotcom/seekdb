...
int ObFTParserProperty::parse_for_parser_helper(const ObFTParser &parser, const ObString &json_str)
{
  ...
  if (OB_FAIL(props.init())) {
    ...
    if (parser.is_ik()) {
      // The strings returned by ObFTParserJsonProps are owned by props. Copy
      // them before props is destroyed instead of keeping dangling ObStrings.
      ObString table_name;
      if (OB_FAIL(props.config_get_dict_table(table_name))) {
        if (OB_SEARCH_NOT_FOUND == ret) {
          ret = dict_table_.assign(ObFTSLiteral::FT_DEFAULT_IK_DICT_UTF8_TABLE);
        }
      ...
      if (OB_SUCC(ret) && OB_FAIL(props.config_get_stopword_table(table_name))) {
        if (OB_SEARCH_NOT_FOUND == ret) {
          ret = stopword_table_.assign(ObFTSLiteral::FT_DEFAULT_IK_STOPWORD_UTF8_TABLE);
        }
      ...
      if (OB_SUCC(ret) && OB_FAIL(props.config_get_quantifier_table(table_name))) {
        if (OB_SEARCH_NOT_FOUND == ret) {
          ret = quantifier_table_.assign(ObFTSLiteral::FT_DEFAULT_IK_QUANTIFIER_UTF8_TABLE);
        }
